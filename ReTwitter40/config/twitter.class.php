...
/// reply // إضافة احمد المالكي

	public function reply($message,$tweetid,$userS)
	{
		if (iconv_strlen($message, 'UTF-8') > 140) {
			$message = preg_replace_callback('#https?://\S+[^:);,.!?\s]#', array($this, 'shortenUrl'), $message);
		}
   //     "in_reply_to_screen_name": "UberFacts", 
    //    "in_reply_to_status_id": 321431587684245506, 
   //     "in_reply_to_status_id_str": "321431587684245506", 
		@$res = $this->request('statuses/update', array('status' => $message,'in_reply_to_status_id' => $tweetid,'in_reply_to_status_id_str' => $tweetid,'in_reply_to_screen_name' => $userS));
		if(@$res->id) {return @$res->id;}else { return 0;}
	}
//user_timeline  // إضافة احمد المالكي
	public function user_timeline($screen_name)
	{
//	echo	$this->consumerKey ;
//	echo	$this->consumerSecret  ;
//	echo	$this->accessToken  ;		
//	echo	$this->accessTokenSecret  ;

$url = 'https://api.twitter.com/1.1/statuses/user_timeline.json';
//$screen_name    = 'Ahmed_Al_malky'; // This is part of your request's query
$count = 1; // This is part of your request's query
...
$oauth_vals[] = "screen_name={$screen_name}";

$base = 'GET&' . rawurlencode($url) . '&' . rawurlencode(implode('&', $oauth_vals));
$composite_key      = rawurlencode($this->consumerSecret) . '&' . rawurlencode($this->accessTokenSecret);
...
$options = array(
    ...
    CURLOPT_URL             => "https://api.twitter.com/1.1/statuses/user_timeline.json?count={$count}&screen_name={$screen_name}",
...
//array('id_str' => $twitter_data['0']['id_str'] , 'id_user' => $twitter_data['0']['id_str'] );
return $twitter_data['0'];
	}
	/**
